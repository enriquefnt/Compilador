...
# Verificar tamaño y comprimir si > 3 MB
            size_bytes = os.path.getsize(output_pdf_dest)
            size_mb = size_bytes / (1024 * 1024)  # Convertir a MB para facilitar comparación
            print(f"Tamaño del PDF final: {size_bytes} bytes ({size_mb:.2f} MB)")
            if size_mb > 10:  # Mayor a 10 MB
                compression_level = "screen"
                print("PDF supera 10 MB. Usando compresión 'screen' (más agresiva).")
            elif size_mb > 3:  # Mayor a 3 MB pero <= 10 MB
                compression_level = "ebook"
                print("PDF supera 3 MB. Usando compresión 'ebook'.")
            else:  # <= 3 MB
                compression_level = "none"
                print("PDF no supera 3 MB; no se comprime.")
            if compression_level != "none":
                print("Iniciando compresión...")
                compressed_temp_path = output_pdf_dest.replace('.pdf', '_compressed_temp.pdf')
                final_compressed_path = compress_pdf(output_pdf_dest, compressed_temp_path, compression_level)
                if final_compressed_path != output_pdf_dest:  # Si se comprimió exitosamente
                    os.remove(output_pdf_dest)  # Eliminar el original
                    shutil.move(final_compressed_path, output_pdf_dest)  # Reemplazar con el comprimido
                    print(f"PDF comprimido y reemplazado: {output_pdf_dest}")
                else:
                    print("Compresión fallida o no redujo tamaño; manteniendo original.")
            else:
                print("No se requiere compresión.")

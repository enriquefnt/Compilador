...
GS_EXECUTABLE_REL = "gs/gs10.05.1/bin/gswin64c.exe"
...
def check_dependencies():
    ...
    try:
        gs_path = get_resource_path(GS_EXECUTABLE_REL)
    except Exception:
        gs_path = None

    if not gs_path or not os.path.exists(gs_path):
        messagebox.showwarning("Advertencia", f"Ghostscript no encontrado ({gs_path}). La compresión no funcionará.")
    try:
...
def compress_pdf(input_path, output_path, compression_level="none"):
    ...
    # Obtener ruta correcta del ejecutable GS dentro del paquete
    gs_executable = get_resource_path(GS_EXECUTABLE_REL)

    if not os.path.exists(gs_executable):
        print(f"Ghostscript no encontrado en {gs_executable}.")
        return input_path
